...
ALL_INDICATORS = {
    ...
    'MNCH_ANC4':            ('Layer 3', 'ANC 4+ Visits'),
    'MNCH_SAB':             ('Layer 3', 'Skilled Birth Attendance'),
    'IM_DTP3':              ('Layer 3', 'DTP3 Coverage'),
    'NT_SAM_TR':            ('Layer 3', 'SAM Treatment'),
    'NT_VAS_12_59':         ('Layer 3', 'Vitamin A Supp.'),
}
...
print(f"Loaded: {len(df_sdmx):,} rows, {df_sdmx['iso3'].nunique()} countries")

# =============================================================================
# FIGURE 8: Comprehensive data gap heatmap
# =============================================================================
...
ax.set_title(
    ...
    'UNICEF Conceptual Framework: All Layers (UNICEF SDMX data, 2024)',
...
LAYER_WEIGHTS = {'Layer 1': 0.34, 'Layer 2': 0.33, 'Layer 3': 0.33}
...
for country_iso in df_sdmx['iso3'].unique():
    ...
    country_scores.append({
        ...
        'layer3_score':     scores_by_layer.get('Layer 3', 0),
        'nis_readiness_score': total_score,
    })

df_scores = pd.DataFrame(country_scores).sort_values('nis_readiness_score', ascending=False)
...
# NIS Readiness by Region chart
REGION_COLORS = {
    'Sub-Saharan Africa':           UNICEF_BLUE,
...
colors = [REGION_COLORS.get(r, '#AAAAAA') for r in region_score['region']]
...
ax.set_yticklabels(region_score['region'].str.replace(' and ', ' & '), fontsize=9)
ax.set_xlabel('Median NIS Readiness Score\n(% of indicators with recent data ≤5 years)', fontsize=10)
ax.set_title(
    'National Nutrition Information System Readiness by Region\n'
    'Based on Data Recency Across GNMF Indicator Set',
    fontweight='bold'
)
ax.axvline(50, color='red', linestyle='--', alpha=0.5, linewidth=1.2, label='50% threshold')
ax.set_xlim(0, 115)
ax.legend(fontsize=8)
plt.tight_layout()
plt.savefig(f'{FIGURES}/fig09_nis_readiness_by_region.png', bbox_inches='tight', dpi=150)
plt.show()
plt.close()
...
tableau_master = tableau_master.merge(
    df_scores[['iso3', 'nis_readiness_score', 'layer1_score',
...
key_findings = pd.DataFrame([
    {'Finding': 'Global stunting still 23.2% (2024)',
     ...
     'Implication': 'Pace of decline insufficient for 2030 target'},
    ...
    {'Finding': 'Data gaps largest for SAM treatment coverage',
     'Layer': 'Layer 3', 'Indicator': 'SAM Treatment',
    ...
    {'Finding': 'Enabling environment data most fragmented',
     'Layer': 'Layer 4', 'Indicator': 'Policy/governance',
     'Source': 'WHO GINA, SUN, ILO',
